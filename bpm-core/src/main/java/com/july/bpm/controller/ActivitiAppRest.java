...
import com.july.bpm.config.ApiResult;
import com.july.bpm.entity.FlowForm;
import com.july.bpm.entity.KeyValue;
...
public class ActivitiAppRest {
    ...
    public String createModel(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            //response.sendRedirect(request.getContextPath() + "/static/flow-editor/modeler.html?modelId=" + id);
    ...
    @ResponseBody
    public ApiResult deploymentModel(String id) throws Exception {
        ...
        if (bytes == null) {
            return ApiResult.fail("模型数据为空，请先设计流程并成功保存，再进行发布。");
        }
        JsonNode modelNode = modelNode = new ObjectMapper().readTree(bytes);

        BpmnModel model = new BpmnJsonConverter().convertToBpmnModel(modelNode);
        if (model.getProcesses().size() == 0) {
            return ApiResult.fail("数据模型不符要求，请至少设计一条主线流程。");
        }
    ...
    @ResponseBody
    public ApiResult startProcesses(@RequestParam Map<String, Object> param) {
...
//        }
        ...
        if (null == pi) {
            return ApiResult.fail("流程启动失败！");
        }
        return ApiResult.ok("启动流程成功！");
    ...
    @ResponseBody
    public ApiResult completeTasks(@RequestParam Map<String, Object> param) {
     ...
     **/
    public List<FlowForm> assembleProcessForm(String processInstanceId) {
        ...
        List<FlowForm> myform = new ArrayList<>();
    ...
    @ResponseBody
    public ApiResult generateProcessImg(String processInstanceId) throws IOException {
        //获取历史流程实例
        ...
        bos.close();
        imageStream.close();
        return ApiResult.ok("success", png_base64);
    }
